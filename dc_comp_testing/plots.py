...
from data_processing import split_curve, fft_analysis
...
def fft_plots(cftimes, rtimes, filterforce, force, relres):
    
    # extract force, filterforce, relres data
    ff_freq, ff_mag = fft_analysis(cftimes, filterforce)
    force_freq, force_mag = fft_analysis(cftimes, force)
    relres_freq, relres_mag = fft_analysis(rtimes, relres)
    
    # relres and force plots
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(15, 5))

    # left: force plots x2
    ax1.plot(ff_freq, ff_mag, label='Filtered force')
    ax1.plot(force_freq, force_mag, label='Force')
    ax1.set_title('Force FFT')
    ...
    # right: relres plot
    ax2.plot(relres_freq, relres_mag, color='crimson', linewidth=1.5)
    ax2.set_title('Relres FFT')
...
def plot_time_series(sample, ftimes, force, rxtimes, relr):
    
    fig, ax1 = plt.subplots(layout="constrained")
    fig.canvas.manager.set_window_title(f"{sample} - DC - Force and relative resistance vs time")
    line1, = ax1.plot(ftimes, force, color='blue', label='Force (g)')
    ax1.set_title(f"{sample} - DC - Force and relative resistance vs time")
    ax1.set_ylabel('Force (g)')
    ax1.set_xlabel('Time (ms)')
    ax2 = ax1.twinx()
    line2, = ax2.plot(rxtimes, relr, color='red', label='Relative resistance', alpha=0.5)
    ax2.set_ylabel('Relative Resistance')

def plot_hysteresis_loop(sample, channel, avg_force, avg_relx,
                         leaderboards_up, leaderboards_down,
                         lower_bound, changeover_bound, hyst_ratio_x):

    plt.figure(f"{sample} - DC - Relative {channel} vs force", layout="constrained")
    plt.plot(avg_force, avg_relx, color='blue', alpha=0.5, label='Actual Loop Data')
    ...
    # text boxes
    plt.text(avg_force[0], avg_relx[0], ' Start ', ha='left', va='center', fontsize=12, color='red')
    plt.text(avg_force[-1], avg_relx[-1], ' End', ha='left', va='center', fontsize=12, color='red')
    plt.title(f"{sample} - DC - Relative {channel} vs force")
